...
function te = read_episodes_from_file(path, file
    ...
    %trajectory_observations = trajectory_observations(:,[4, 3, 9, 18, 23, 21, 30]);%Long, lat, datetime, rail type, street type, street maxspeed, building typ
    trajectory_observations = trajectory_observations(:,[1,2,11]); %Row, column, time (numeric

    r2_y = table2array(trajectory_observations(:,1)); %sub2ind says it's row, column, but we need to do column, row, because index is transposed between R and matla
    ...
    for e = 1:trajectory_episodes_coun
