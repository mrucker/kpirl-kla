...
daps = {
    ...
    'kla_spd', @kla_spd, struct('N', 30, 'M', 90, 'T', 03, 'v_feats', 1, 'W', 03);

    %generate a policy using kla_mem and features 1 (this kla implementation decreases memory use by increasing computation)
    %'kla_mem', @kla_mem, struct('N', 10, 'M', 90, 'T', 03, 'v_feats', 1, 'W', 03);

    %generate a policy using lspi and features 1 with a second order polynomial transform applied to the basis 
    ...
    'klspi'  , @klspi  , struct('N', 30, 'M', 90, 'T', 06, 'v_feats', 1, 'resample', true, 'kernel', k_gaussian(k_norm(),1.2), 'mu', 0.2);    
}';
